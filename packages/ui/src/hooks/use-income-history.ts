 ...
 * this month normal?".
 *
 * It reuses the SAME query key the dashboard's monthly report uses, so the
 * month already on screen costs nothing to add here — react-query serves it
 * from cache instead of refetching it. That is the whole reason this is six
