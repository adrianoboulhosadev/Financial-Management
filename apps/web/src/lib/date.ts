...
/**
 * Formats a date as Brazilian date + time, e.g. "25/07/2026 18:30". Used for
 * timestamps (when something was recorded), never for the day a movement is
 * filed under — that one is a date, and rendering it in local time could show
 * the previous day.
 */
...
/**
 * The value an <input type="date"> expects: "YYYY-MM-DD", read in UTC. The API
 * sends day-granularity dates as UTC midnight, so reading them locally would
 * shift the field to the previous day for anyone west of Greenwich.
 */
